...
public class ApiSyntheticMonitoring extends BaseClass {
	...
	@Test(enabled = true)
	void placeGTTOrder_Modify_Cancel_Buy_Equity() throws IOException, InterruptedException {

		// get LTP price to manipulate Limit Price
		String pLtp = baseAPI.getLTPPriceGTT("11536", "nse_cm");
		String ltpPrice = helper.gttBuyCancelOrderLimitPrice(pLtp);
		System.out.println("Post rounding off LTP value = " + ltpPrice);
		String triggerPrice = helper.buyValueTriggerPriceForGTT(ltpPrice);
		System.out.println("Post rounding off triggerPrice value = " + triggerPrice);
		// String variety = helper.orderTypeCheckForEquity();
		// Place Limit Orders
		Response response = baseAPI.placeGttOrder(cDetails.getClientId(), "nse_cm", ltpPrice, "CNC", "1", "11536",
				"TCS-EQ", triggerPrice);
		validateAPIResponse(response);
		String orderNum = response.jsonPath().getString("data.id");
		Thread.sleep(2000);
		Response gttOrderStatus = baseAPI.getGttOrderStatus(orderNum);
		validateAPIResponse(gttOrderStatus);
		String status = gttOrderStatus.jsonPath().getString("data.status");
		String createDate = gttOrderStatus.jsonPath().getString("data.createddate");
		String expirydate = gttOrderStatus.jsonPath().getString("data.expirydate");
		String symbolName = gttOrderStatus.jsonPath().getString("data.symbolName");
		String desc = gttOrderStatus.jsonPath().getString("data.desc");
		Integer price = gttOrderStatus.jsonPath().getInt("data.price");
		Double qty = gttOrderStatus.jsonPath().getDouble("data.qty");
		Integer triggerprice = gttOrderStatus.jsonPath().getInt("data.triggerprice");
		String exchange = gttOrderStatus.jsonPath().getString("data.exchange");
		Assertions.assertThat(status).as("totalBalance shouldn't be null").isNotBlank().isNotNull().isNotEmpty()
				.isEqualTo("1");
		Assertions.assertThat(createDate).as("totalBalance shouldn't be null").isNotBlank().isNotNull().isNotEmpty();
		Assertions.assertThat(expirydate).as("totalBalance shouldn't be null").isNotBlank().isNotNull().isNotEmpty();
		Assertions.assertThat(symbolName).as("totalBalance shouldn't be null").isNotBlank().isNotNull().isNotEmpty();
		Assertions.assertThat(desc).as("totalBalance shouldn't be null").isNotBlank().isNotNull().isNotEmpty();
		Assertions.assertThat(price).as("totalBalance shouldn't be null").isNotNull().isNotNegative();
		Assertions.assertThat(qty).as("totalBalance shouldn't be null").isNotNull().isNotNegative();
		Assertions.assertThat(triggerprice).as("totalBalance shouldn't be null").isNotNull().isNotNegative();
		Assertions.assertThat(exchange).as("totalBalance shouldn't be null").isNotBlank().isNotNull().isNotEmpty();
		Thread.sleep(2000);

		// Modify Gtt Order
		Response modifyGttOrder = baseAPI.modifyGttOrder(exchange, orderNum, ltpPrice, "1", "11536", triggerPrice);
		validateAPIResponse(modifyGttOrder);

		// cancel Gtt Order
		Response callGttCancelOrderApi = baseAPI.callGttCancelOrderApi(orderNum, "11536", "nse_cm");
		validateAPIResponse(callGttCancelOrderApi);
		Thread.sleep(2000);
		Response gttOrderStatusPostCancel = baseAPI.getGttOrderStatus(orderNum);
		String cancelStatus = gttOrderStatusPostCancel.jsonPath().getString("data.status");
		Assertions.assertThat(cancelStatus).as("totalBalance shouldn't be null").isNotBlank().isNotNull().isNotEmpty()
				.isEqualTo("3");
	}
	...
	void testOrderStatus() throws IOException, InterruptedException {
		...
		Response orderStatus = baseAPI.getOrderStatus("230517000260310", "230517000260310");

//		Response callGttOrderListApi = baseAPI.callGttOrderListApi();
//		JSONObject object = new JSONObject(callGttOrderListApi.asString());
//		JSONArray list = object.getJSONArray("data");
//		for (int i = 0; i < list.length(); i++) {
//			String orderNum = String.valueOf(list.getJSONObject(i).getInt("id"));
//			Response callGttCancelOrderApi = baseAPI.callGttCancelOrderApi(orderNum,"11536","nse_cm");
//		}
	...
	private void checkOrderStatus(String orderNum) {
		Response orderStatusResponse = baseAPI.getOrderStatus(orderNum, orderNum);
		String orderStatus = orderStatusResponse.jsonPath().getString("data.status");

		if (orderStatus.contains("open") || orderStatus.contains("after market order"))
			ExtentLogger.info(orderNum + " Order Id found in open order");
		else if (orderStatus.contains("complete"))
			ExtentLogger.info(orderNum + " Order got executed");
		else {
			ExtentLogger.fail(orderNum + " Order Id not found in open order.Probably rejected.Please check manually");
			Assert.fail("Place Order isnt successful as order Id " + orderNum + " doesnt found in Open Order");
		}
	...
	void placeOrder_Buy_CARRYFORWARD_LimitOrder_Currency() throws IOException, InterruptedException {
		...
		Thread.sleep(1000);
		// checkOrderStatus
		String orderStatus = checkOrderStatusIgnoreFund(orderNum);
		// Cancel Order
		cancelAndVerify(variety, orderNum, orderStatus);

	}

	private String checkOrderStatusIgnoreFund(String orderNum) {
		Response orderStatusResponse = baseAPI.getOrderStatus(orderNum, orderNum);
		String orderStatus = orderStatusResponse.jsonPath().getString("data.status");
		String orderText = orderStatusResponse.jsonPath().getString("data.text");
		if (orderStatus.contains("open") || orderStatus.contains("after market order"))
			ExtentLogger.info(orderNum + " Order Id found in open order");
		// System.out.println(" Order Id found in open order");
		else if (orderStatus.contains("rejected") && orderText.contains("Insufficient Funds")) {
	...
	public void testWithdrawalBalanceApi() throws Exception {
		...
		Assertions.assertThat(totalBalance).as("totalBalance shouldn't be null").isNotBlank().isNotNull().isNotEmpty()
				.isNotEqualTo("0");
		Assertions.assertThat(releasbleAmount).as("releasbleAmount shouldn't be null").isNotBlank().isNotNull()
				.isNotEmpty().isNotEqualTo("0");
		Assertions.assertThat(unsettledBalance).as("unsettledBalance shouldn't be null").isNotBlank().isNotNull()
				.isNotEmpty();
	}
	...
	public void testWithdrawListApi() throws Exception {
		...
		Assertions.assertThat(transactionAmount).as("transactionAmount shouldn't be null").isNotBlank().isNotNull()
				.isNotEmpty().isNotEqualTo("0");
		Assertions.assertThat(transactionDate).as("transactionDate shouldn't be null").isNotBlank().isNotNull()
				.isNotEmpty().isNotEqualTo("0");
		Assertions.assertThat(transactionID).as("trabnsactionID shouldn't be null").isNotBlank().isNotNull()
				.isNotEmpty();
	}
	...
	public void testTransactionMergedListApi() throws Exception {
		...
		Assert.assertTrue(transactionMergedList.getStatusCode() == 200,
				"Invalid Response for gettransactionmerged list API");
	...
	@Test(enabled = true)
	public void testExitAll() throws Exception {
		Response placeStockOrder = null;
		Response positions = baseAPI.getPositions();
		JSONObject object = new JSONObject(positions.asString());
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			String symboltoken = jsonArray.getJSONObject(i).getString("symboltoken");
			String tradingsymbol = jsonArray.getJSONObject(i).getString("tradingsymbol");
			String buyqty = jsonArray.getJSONObject(i).getString("buyqty");
			String sellqty = jsonArray.getJSONObject(i).getString("sellqty");
			String cfbuyqty = jsonArray.getJSONObject(i).getString("cfbuyqty");
			String cfsellqty = jsonArray.getJSONObject(i).getString("cfsellqty");
			String netqty = jsonArray.getJSONObject(i).getString("netqty");
			netqty = removeSpecialCharFromInt(netqty);

			 if(!netqty.equalsIgnoreCase("0")) {
				 int buyqty1 = Integer.valueOf(buyqty);
				 int sellqty1 = Integer.valueOf(sellqty);
				 int cfbuyqty1 = Integer.valueOf(cfbuyqty);
				 int cfsellqty1 = Integer.valueOf(cfsellqty);
				 
				if (buyqty1<sellqty1) {

					placeStockOrder = baseAPI.placeStockOrder(exchange, "MARKET", "0.0", producttype, netqty, "0",
							symboltoken, tradingsymbol, "BUY", "0.0", "NORMAL");
				} else if(buyqty1>sellqty1){
					placeStockOrder = baseAPI.placeStockOrder(exchange, "MARKET", "0.0", producttype, netqty, "0",
							symboltoken, tradingsymbol, "SELL", "0.0", "NORMAL");
				} 
				if (cfbuyqty1<cfsellqty1) {

					placeStockOrder = baseAPI.placeStockOrder(exchange, "MARKET", "0.0", producttype, netqty, "0",
							symboltoken, tradingsymbol, "BUY", "0.0", "NORMAL");
				} else if(cfbuyqty1>cfsellqty1){
					placeStockOrder = baseAPI.placeStockOrder(exchange, "MARKET", "0.0", producttype, netqty, "0",
							symboltoken, tradingsymbol, "SELL", "0.0", "NORMAL");
				} 
	...
	public static String removeSpecialCharFromInt(String text) {
		Pattern pattern = Pattern.compile("\\d+\\.\\d+");
		String valueWithoutSpecialChar = text.replaceAll("[^\\d]", "");
		return String.valueOf(valueWithoutSpecialChar);
	}
	...
	public void testMCXChartsApi() {
		...
		Response mcxChartsApiRespoonse = baseAPI.getMCXCharts(1, "Req", mcxTopic, "OHLCV", "I", durationType, 1,
				startTime, endTime);
	...
	private void verifyOrderIsCancelled(String orderNum, String variety) {
		Response orderStatusResponse = baseAPI.getOrderStatus(orderNum, orderNum);
		String orderStatus = orderStatusResponse.jsonPath().getString("data.ord_status");
		if (variety.equalsIgnoreCase("NORMAL"))
			Assertions.assertThat(orderStatus).as("Order Id isn't cancelled").isEqualToIgnoringCase("cancelled");
		else
			Assertions.assertThat(orderStatus).as("Order Id isn't cancelled").isEqualToIgnoringCase("AMO CANCELLED");

	}

	private void cancelAndVerify(String variety, String orderNum, String orderDetails) throws InterruptedException {
		if (orderDetails.equalsIgnoreCase("open") || orderDetails.contains("after market order")) {
			Response cancelOrderResponse = baseAPI.cancelOrder(orderNum, variety);
