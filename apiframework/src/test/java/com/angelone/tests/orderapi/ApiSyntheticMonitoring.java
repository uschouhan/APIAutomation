...
class ApiSyntheticMonitoring extends BaseClass {
    private static final List<String> BUILDUPS = new ArrayList<>(Arrays.asList("Long Built Up", "Short Built Up", "Short Covering", "Long Unwinding"));
    private static final List<String> DERIVATIVE_MARKET = new ArrayList<>(Arrays.asList("PercPriceGainers", "PercPriceLosers", "PercOIGainers", "PercOILosers"));
    private static final List<String> MARKET_MOVERS_DATA = new ArrayList<>(Arrays.asList("TOPGAINER", "TOPLOSER"));
    private static final List<String> MARKET_MOVERS_BY_MOST_DATA = new ArrayList<>(Arrays.asList("MOST_ACT_VOLUMN", "MOST_ACT_VALUE"));
    ...
    public void testPortfolioAdvisoryApi() throws Exception {
        ...
        Assertions.assertThat(portfolioAdvioryData.jsonPath().getString("status")).as("portfolioAdviory status isn't showing success")
                .isEqualTo("success");
    ...
    public void testIpoDetails() throws Exception {
        ...
        if (list.size() > 0) {
            ...
            Assertions.assertThat(companyDesription).as("companyDesription shouldn't be null").isNotBlank().isNotNull().isNotEmpty();
            Assertions.assertThat(companyStrength).as("companyStrength shouldn't be null").isNotBlank().isNotNull().isNotEmpty();
            Assertions.assertThat(companyRisks).as("companyRisks shouldn't be null").isNotBlank().isNotNull().isNotEmpty();
            ...
            Assertions.assertThat(maxTotalAmt).as("maxTotalAmt shouldn't be null").isNotBlank().isNotNull().isNotEmpty();
            Assertions.assertThat(minimumBidLotQty).as("minimumBidLotQty shouldn't be null").isNotBlank().isNotNull().isNotEmpty();
            Assertions.assertThat(maximumBidLotQty).as("maximumBidLotQty shouldn't be null").isNotBlank().isNotNull().isNotEmpty();
    ...
    void testFundamentalRatioApi() throws IOException {
        ...
        Assert.assertTrue(!FundamentalRatio.jsonPath().getString("data").isEmpty(), "data is empty in FundamentalRatio api ");
        String data = FundamentalRatio.jsonPath().getString("data");
        String decodeJsonResponse = baseAPI.decodeJsonResponse(data);
        System.out.println("Market Data - " + decodeJsonResponse);
        System.out.println(" FundamentalRatio data \n" + decodeJsonResponse);
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            Assertions.assertThat(Name).as("Name is null for FundamentalRatioApi").isNotEmpty().isNotNull().isNotBlank();
            Assertions.assertThat(Value).as("Value is null for FundamentalRatioApi").isNotEmpty().isNotNull().isNotBlank();
        }
    }
    ...
    void testFutureBuildUpHeatMap() throws IOException {
        BUILDUPS.forEach(buildup -> {
                    ...
                    Assert.assertTrue(!marketBuildupResponse.jsonPath().getString("data").isEmpty(), "data is empty in marketBuildupResponse api ");
                    String data = marketBuildupResponse.jsonPath().getString("data");
                    String decodeJsonResponse = baseAPI.decodeJsonResponse(data);
                    System.out.println(buildup + " data \n" + decodeJsonResponse);
                    JSONArray jsonArray = new JSONArray(decodeJsonResponse);
                    for (int i = 0; i < jsonArray.length(); i++) {
                        JSONObject object = jsonArray.getJSONObject(i);
                        ...
                        Assertions.assertThat(String.valueOf(NetChg)).as("NetChg is null for " + buildup).isNotEmpty().isNotNull().isNotBlank();
                        Assertions.assertThat(String.valueOf(Ltp)).as("Ltp is null for " + buildup).isNotEmpty().isNotNull().isNotBlank();
                        Assertions.assertThat(SecurityDesc).as("SecurityDesc is null for " + buildup).isNotEmpty().isNotNull().isNotBlank();
                    }
                }
        );
    }
    ...
    void testFutureMarketsIndicatorsDerivativeMarketApi() throws IOException {
        DERIVATIVE_MARKET.forEach(buildup -> {
                    ...
                    Assert.assertEquals(FutureMarketsIndicators.jsonPath().getString("status"), "success",
                            "Status doesnt match in marketBuildup api " + buildup);
                    Assert.assertTrue(!FutureMarketsIndicators.jsonPath().getString("data").isEmpty(), "data is empty in FutureMarketsIndicators api ");
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        Assertions.assertThat(StockXChangeCode).as("StockXChangeCode is null for " + buildup).isNotEmpty().isNotNull().isNotBlank();
                        Assertions.assertThat(EnumName).as("EnumName is null for " + buildup).isNotEmpty().isNotNull().isNotBlank();
                        Assertions.assertThat(String.valueOf(Ltp)).as("Ltp is null for " + buildup).isNotEmpty().isNotNull().isNotBlank();
                        Assertions.assertThat(ExpiryDate).as("ExpiryDate is null for " + buildup).isNotEmpty().isNotNull().isNotBlank();
                    }
                }
        );
    }
    ...
    void testDerivativeMarketSectorHeatMap() throws IOException {
        ...
        String decodeJsonResponse = baseAPI.decodeJsonResponse(data);
        System.out.println("sectorHeatMap data \n" + decodeJsonResponse);
        JsonPath jPath = new JsonPath(decodeJsonResponse);
        ...
        Assertions.assertThat(IndexName).as("IndexName is null for sectorHeatMap").isNotEmpty().isNotNull().isNotBlank();
        Assertions.assertThat(String.valueOf(SectorName)).as("NetChg is null for sectorHeatMap").isNotEmpty().isNotNull().isNotBlank();
        Assertions.assertThat(String.valueOf(PercChg)).as("PercChg is null for sectorHeatMap").isNotEmpty().isNotNull().isNotBlank();
        Assertions.assertThat(PercChgOI).as("PercChgOI is null for sectorHeatMap").isNotEmpty().isNotNull().isNotBlank();
        Assertions.assertThat(PercChgVolume).as("PercChgVolume is null for sectorHeatMap").isNotEmpty().isNotNull().isNotBlank();
        Assertions.assertThat(PercRollover).as("PercRollover is null for sectorHeatMap").isNotEmpty().isNotNull().isNotBlank();
    ...
    void testInstaTradeDetailsApi() throws IOException {
        ...
        Assert.assertTrue(!IntraTradeResponse.jsonPath().getString("data").isEmpty(), "data is empty in IntraTrade api ");
        ...
        Assertions.assertThat(jPath.getString("data.symbolList[0].symbolDesc")).as("Symbol desc is blank/null").isNotBlank().isNotEmpty().isNotNull();
        Assertions.assertThat(jPath.getString("data.symbolList[0].symbolDetails.symbolName")).as("symbolName is blank/null").isNotBlank().isNotEmpty().isNotNull();
        Assertions.assertThat(jPath.getString("data.symbolList[0].symbolDetails.tradeSymbol")).as("tradeSymbol is blank/null").isNotBlank().isNotEmpty().isNotNull();
        Assertions.assertThat(jPath.getString("data.symbolList[0].symbolDetails.symbolDesc")).as("symbolDesc is blank/null").isNotBlank().isNotEmpty().isNotNull();
        Assertions.assertThat(jPath.getString("data.symbolList[0].symbolDetails.tokenID")).as("tokenID is blank/null").isNotBlank().isNotEmpty().isNotNull();
        Assertions.assertThat(jPath.getString("data.symbolList[0].symbolDetails.expiry")).as("expiry is blank/null").isNotBlank().isNotEmpty().isNotNull();
        Assertions.assertThat(jPath.getString("data.symbolList[0].symbolDetails.details")).as("details is blank/null").isNotBlank().isNotEmpty().isNotNull();
        Assertions.assertThat(jPath.getString("data.symbolList[0].symbolDetails.astCls")).as("astCls is blank/null").isNotBlank().isNotEmpty().isNotNull();
        Assertions.assertThat(jPath.getString("data.symbolList[0].symbolDetails.optType")).as("optType is blank/null").isNotBlank().isNotEmpty().isNotNull();
        Assertions.assertThat(jPath.getString("data.symbolList[0].symbolDetails.minLot")).as("minLot is blank/null").isNotBlank().isNotEmpty().isNotNull();
        Assertions.assertThat(jPath.getString("data.symbolList[0].symbolDetails.regLot")).as("regLot is blank/null").isNotBlank().isNotEmpty().isNotNull();
    }
    ...
    public void testTopGainerNLoserApi() throws Exception {
        MARKET_MOVERS_DATA.forEach(queryParam -> {
            ...
            Assertions.assertThat(symbolName).as("symbolName is null for market movers "+queryParam).isNotEmpty().isNotNull().isNotBlank();
            Assertions.assertThat(String.valueOf(details)).as("details is null for market movers "+queryParam).isNotEmpty().isNotNull().isNotBlank();
            Assertions.assertThat(String.valueOf(ltp)).as("ltp is null for market movers "+queryParam).isNotEmpty().isNotNull().isNotBlank();
            Assertions.assertThat(change).as("change is null for market movers "+queryParam).isNotEmpty().isNotNull().isNotBlank();
            Assertions.assertThat(changePer).as("changePer is null for market movers "+queryParam).isNotEmpty().isNotNull().isNotBlank();
        });
    }
